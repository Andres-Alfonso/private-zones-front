...
export const meta: MetaFunction = () => {
  return [
    { title: "Gestión de Foros - Admin Panel" },
    { name: "description", content: "Gestión de foros de discusión" },
  ];
};
...
export default function ForumsLayout() {
  return (
    <AuthGuard>
      <RoleGuard requiredRoles={['superadmin', 'admin', 'instructor', 'student']} requireAll={false}>
        <ForumsLayoutContent />
      </RoleGuard>
    </AuthGuard>
  );
...
function ForumsLayoutContent() {
  ...
  const isDetailPage = location.pathname.match(/\/forums\/[^\/]+$/) && !isCreatePage;
  const isIndexPage = location.pathname === '/forums';
  const isCourseForumsPage = location.pathname.includes('/forums/course/');

  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50">
      {/* Header del módulo de foros */}
                ...
                <nav className="flex items-center space-x-2 text-sm text-gray-500 mb-3">
                  ...
                  {course ? (
                    <>
                      ...
                      <span className="text-gray-300">/</span>
                      <span className="text-blue-600 font-semibold">Foros</span>
                    </>
                  ) : (
                    <span className="text-gray-900 font-semibold">Foros</span>
                  )}
                  {isCreatePage && (
                    <>
                      <span className="text-gray-300">/</span>
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    <h1 className="text-3xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent">
                      {course ? `Foros - ${course.translations?.[0]?.title}` : 'Gestión de Foros'}
                    </h1>
                    <p className="text-gray-600 mt-1 text-lg">
                      {course ? 'Foros de discusión del curso' : 'Gestión general de foros'}
                    </p>
                  </div>
                </div>
              </div>

              {/* Botón de volver si estamos en foros de un curso */}
              {course && (
                <div className="ml-4">
                  <NavLink
        ...
        <div className="container mx-auto px-4 py-8">
          <div className="flex items-center justify-between text-sm">
            <div className="font-medium text-gray-700">
              © {new Date().getFullYear()} Plataforma de Foros
            </div>
